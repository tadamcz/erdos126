...
pre-release version of **GPT-6 Astra** (OpenAI) in the **FrontierMath Erdős** benchmark (Epoch AI and Thomas F. Bloom, 2026). The
...
**Benchmark.** FrontierMath Erdős evaluates AI systems on 68 open Erdős problems selected by Thomas F. Bloom, in the Lean proof
assistant, autonomously and under a fixed budget ($300 and 72 hours of working time per attempt in the default configuration). The
...
Costs are as metered by the harness at GPT-5.6 Sol per-token prices (the pre-release model's prices were unpublished at the time of the
runs; the paper recomputes them at GPT-6 Astra's actual prices, roughly 1.8–2.0× higher). "Default configuration" is the deepagent-based
agent with subagents, memory and an offline arXiv snapshot under the $300 / 72 h budget; "ReAct agent, larger budget" is a basic agent
with a larger budget. The Inspect transcripts are linked for the record (access may be restricted).

| Module | Role | Attempt | Metered cost | Working time | Inspect log |
|---|---|---|---|---|---|
| `Erdos126/Resolutions/Erdos126_132usd_25h.lean` | **primary** (wired to `Solution.lean`) | default configuration, 28 Aug 2026 (benchmark run) | $132 | 25 h | [transcript](https://viewer.hawk.hawkbench.com/permalink/sample/bqPssUuruCU6eDwz4nqbfM) |
| `Erdos126/Resolutions/Erdos126_81usd_13h.lean` | alternate | default configuration, 31 Aug 2026 | $81 | 13 h | [transcript](https://viewer.hawk.hawkbench.com/permalink/sample/DXzbqAfpFPKfbStZgpYCp2) |
| `Erdos126/Resolutions/Erdos126_104usd_15h.lean` | alternate | default configuration, 2 Sep 2026 | $104 | 15 h | [transcript](https://viewer.hawk.hawkbench.com/permalink/sample/i3Mx3xcww5sLaYuDcJ9UdS) |
| `Erdos126/Resolutions/Erdos126_133usd_17h.lean` | alternate | ReAct agent, larger budget, 26 Aug 2026 | $133 | 17 h | [transcript](https://viewer.hawk.hawkbench.com/permalink/sample/WKGPdZNjMC8ehBvW4P6WiD) |
